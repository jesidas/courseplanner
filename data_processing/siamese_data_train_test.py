...
class WORD(object):
    ...
    def load_data(self, n, dataset, pn, datapath):
        ...
        if dataset == 'CISC499 Dataset':
            
            with open('../datasets/CISC499 Dataset/concept_vocab.pkl', 'rb') as fid:
                vocab_ind_dict = cPickle.load(fid)
            all_concepts = set()
            self.x_train, self.y_train, all_concepts = self.get_gt('../datasets/CISC499 Dataset/train_test_5fold/train_'+str(n)+'_index.txt', all_concepts)
            self.x_test, self.y_test, word_inds = self.get_gt('../datasets/CISC499 Dataset/train_test_5fold/test_'+str(n)+'_index.txt', all_concepts)
    ...
    def get_next_batch(self, batch, phase='train', one_shot=False):
        ...
        #for x_ in x1_:
            #if x_!=[]:
                #a = x_.reshape((1,self.dim))
        #x1_ = np.asarray([x_.reshape((1, self.dim)) for x_ in x1_ if x_!=[]])
        x1_ = np.asarray([x_.reshape((1, self.dim)) for x_ in x1_ if x_!=[]])
        #x1_ = np.asarray([a])
        #for x_ in x2_:
            #if x_!=[]:
                #a = x_.reshape((1,self.dim))
        x2_ = np.asarray([x_.reshape((1, self.dim)) for x_ in x2_ if x_!=[]])
        y_ = np.asarray(y_)
        
             
        cs_ = np.asarray([self.vocab[i] for i in cs_ if i in self.vocab.keys()])
        ct_ = np.asarray([self.vocab[i] for i in ct_ if i in self.vocab.keys()])
